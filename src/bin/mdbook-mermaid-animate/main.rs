...
const MERMAID_ANIMATE_JS: &[u8] = include_bytes!("assets/mermaid-animate.js");
const MERMAID_FILES: &[(&str, &[u8])] = &[("mermaid-animate.js", MERMAID_ANIMATE_JS)];

pub fn make_app() -> Command {
    Command::new("mdbook-mermaid")
        .version(crate_version!())
        .about("mdbook preprocessor to add mermaid animation support")
        .subcommand(
...
fn handle_preprocessing() -> Result<(), Error> {
    ...
    log::info!("Processing book with mdbook-mermaid-animate preprocessor");
    let processed_book = MermaidAnimatePreprocessor.run(&ctx, book)?;
...
fn handle_supports(sub_args: &ArgMatches) -> ! {
    ...
    let supported = MermaidAnimatePreprocessor.supports_renderer(renderer);
